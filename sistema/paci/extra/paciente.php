...
<br><br>
          <strong>Tabla de Pacientes</strong>
  ...
  cambiar();
    ...
    var limpiar_datos = function(){
      $("#opcion").val("Registrar");
        ...
        $("#frmeditartipo #file").val("").clone(true);
  ...
  var table = $("#example").DataTable({
    ...
    var obtener_data_editar = function(tbody, table){
      ...
      $(tbody).on("click", "button.editar", function(){
        ...
        var id=$("#frmeditartipo #id").val(data.pa_id), 
        opcion=$("#frmeditartipo #opcion").val("editar"),

        apellidopa=$("#frmeditartipo #apellidopa").val(data.pa_primerapellido),
        apellidoma=$("#frmeditartipo #apellidoma").val(data.pa_segundoapellido),

        nombre_one=$("#frmeditartipo #nombre_one").val(data.pa_primernombre),
        nombre_two=$("#frmeditartipo #nombre_two").val(data.pa_segundonombre),

        fechana=$("#frmeditartipo #fechana").val(data.pa_fechanaci),
        genero=$("#frmeditartipo #genero option[value='"+data.pa_genero+"']").attr('selected', true),

        direccion=$("#frmeditartipo #direccion").val(data.pa_direccion),
        tlffijo=$("#frmeditartipo #tlfijo").val(data.pa_tlfcasa),
        tlfmovil=$("#frmeditartipo #tlfmovil").val(data.pa_tlfmovil),
        email=$("#frmeditartipo #email").val(data.pa_correo),

        foto=$("#frmeditartipo #image_preview").html('<img id="previewing" src="data:image/jpeg; base64,' + data.pa_foto + '" width="100px" height="100px">'),
        inden=$("#frmeditartipo #identification option[value='"+data.iden_id+"']").attr('selected', true),
        dni=$("#frmeditartipo #dni").val(data.pa_dni),
    ...
    var obtener_id_eliminar = function(tbody, table){
      $(tbody).on("click", "button.eliminar", function(){
        var data = table.row( $(this).parents("tr") ).data();
        var idusuario = $("#frmEliminarTipo #id").val( data.pa_id );
      });
    }
    var obtener_id_cambiar = function(tbody, table){
      $(tbody).on("click", "button.estado", function(){
        var data = table.row( $(this).parents("tr") ).data();
        var idusuario = $("#frmestadoTipo #id").val( data.pa_id );
      });
    }
var idioma_espanol = {
