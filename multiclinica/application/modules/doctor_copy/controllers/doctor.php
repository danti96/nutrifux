...
class Doctor extends MX_Controller {
    ...
    public function addNew() {
        ...
        if ($this->form_validation->run() == FALSE) {
            if (!empty($id)) {
                $data = array();
                $data['departments'] = $this->department_model->getDepartment();
                $data['doctor'] = $this->esteticista_model->getDoctorById($id);
                $this->load->view('home/dashboard'); // just the header file
                $this->load->view('add_new', $data);
            ...
            if (empty($id)) {     // Adding New Doctor
                if ($this->ion_auth->email_check($email)) {
                    ...
                    $this->esteticista_model->insertDoctor($data);
                    $doctor_user_id = $this->db->get_where('doctor', array('email' => $email))->row()->id;
                    $id_info = array('ion_user_id' => $ion_user_id);
                    $this->esteticista_model->updateDoctor($doctor_user_id, $id_info);
                ...
                $this->esteticista_model->updateDoctor($id, $data);
    ...
    function getDoctor() {
        ...
        if ($limit == -1) {
            if (!empty($search)) {
                $data['doctors'] = $this->esteticista_model->getDoctorBysearch($search);
            } else {
                $data['doctors'] = $this->esteticista_model->getDoctor();
            }
        } else {
            if (!empty($search)) {
                $data['doctors'] = $this->esteticista_model->getDoctorByLimitBySearch($limit, $start, $search);
            } else {
                $data['doctors'] = $this->esteticista_model->getDoctorByLimit($limit, $start);
            }
        }
        //  $data['doctors'] = $this->esteticista_model->getDoctor();
